...
from utils.unc_data_loader_2_groups import unc_data_loader_2_groups
from utils.unc_original_graph_loader_2g import unc_original_graph_loader_2g
...
def train(args, model):
    ...
    input_graph = unc_original_graph_loader_2g(args)
    model.train()
    ...
    for epoch in range(args.epoch_number):
        ...
        if epoch == 0:
            group_data_transform_ret, hotell2_group_matrix_ret, t2_node_matrix_ret, t2_node_matrix_min_ret, intra_t2_vector_ret = model(
                group_data2gpu, False)
        else:
            group_data_transform_ret, hotell2_group_matrix_ret, t2_node_matrix_ret, t2_node_matrix_min_ret, intra_t2_vector_ret = model(
                group_data2gpu, True)
...
#        print("   intra_t2_vector_ret: ", intra_t2_vector_ret)
        ...
        ttest_loss = 1.0 / (hotell2_group_matrix_ret.mean().log())
...
def test(args, model, epcoh_number):
    ...
    with torch.no_grad():
        multiple_group_data_ret_test = unc_data_loader_2_groups(args)
        ...
        if epcoh_number == 0:
            group_data_transform_ret, hotell2_group_matrix_ret, t2_node_matrix_ret, t2_node_matrix_min_ret, intra_t2_vector_ret = model(
                group_data2gpu_test, False)
        else:
            group_data_transform_ret, hotell2_group_matrix_ret, t2_node_matrix_ret, t2_node_matrix_min_ret, intra_t2_vector_ret = model(
                group_data2gpu_test, True)
